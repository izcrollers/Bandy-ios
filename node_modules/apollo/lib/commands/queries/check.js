        ...
        const tasks = new Listr([
            load_config_1.loadConfigStep(flags, false),
            {
                ...
                task: async (ctx, task) => {
                    ...
                    const operations = loading_1.loadQueryDocuments(ctx.documentSets[0].documentPaths, flags.tagName);
                    task.title = `Scanning for GraphQL queries (${operations.length} found)`;
                    ctx.operations = operations.map(doc => ({ document: graphql_1.print(doc) }));
                }
            },
            {
